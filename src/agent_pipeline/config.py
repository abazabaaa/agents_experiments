...
class PipelineConfig:
    ...
    def from_json(cls, path: Path) -> "PipelineConfig":
        ...
        concurrency_cfg = _coerce_mapping(
            payload.get("concurrency"), name="concurrency"
        )
        ...
        prompts_cfg = _coerce_mapping(payload.get("prompts"), name="prompts")
        rework_cfg = _coerce_mapping(payload.get("rework"), name="rework")
        ...
        for key, spec in agents_cfg.items():
            if not isinstance(spec, Mapping):
                raise TypeError(
                    f"agents.{key} must be a mapping, received {type(spec)!r}"
                )
            agent_specs[str(key)] = AgentSpec.from_payload(
                str(key), spec, base_dir
            )

        # Normalize prompts to a plain {str: str} mapping and optionally load from a directory
        stage_prompts: dict[str, str] = {}

        # 1) Explicit inlined templates under prompts.{stage}
        inline_templates = {k: v for k, v in prompts_cfg.items() if k != "dir"}
        for key, value in inline_templates.items():
            if not isinstance(value, (str, bytes)):
                raise TypeError(
                    f"prompts.{key} must be a string containing the template"
                )
            stage_prompts[str(key)] = str(value)

        # 2) Optional directory loader (prompts.dir). Each *.md file becomes a stage template,
        #    with the stage key equal to the filename stem (e.g., review.md -> "review").
        prompts_dir_value = prompts_cfg.get("dir")
        if prompts_dir_value is not None:
            prompts_dir = Path(str(prompts_dir_value))
            ...
            for p in prompts_dir.iterdir():
                if p.is_file() and p.suffix.lower() in {".md", ".txt"}:
                    ...
                    try:
                        ...
                        raise OSError(f"Failed reading prompt file: {p}") from exc
                    stage_prompts[key] = content

        # Rework policy validation
        rework_enabled = bool(rework_cfg.get("enabled", False))
        rework_max_cycles_raw = rework_cfg.get("max_cycles", 0)
        rework_max_cycles = _coerce_int(
            rework_max_cycles_raw, name="rework.max_cycles", default=0
        )
...
class AgentSpec:
    ...
    attempt_overrides: Mapping[str, "AttemptOverride"]
    ...
    def from_payload(
        ...
        run_max_turns_int = (
            ...
            if isinstance(run_max_turns_val, (int, float)) and not isinstance(run_max_turns_val, bool)
            else None
        )
        ...
        verbosity_value = (
            str(payload.get("verbosity")) if payload.get("verbosity") is not None else None
        )
        if verbosity_value is not None and verbosity_value not in {"low", "medium", "high"}:
            raise ValueError(
