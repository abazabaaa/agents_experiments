---
title: JSON parser - Tutorial (Lark)
description: Build a JSON parser in Python using Lark. Learn to write the grammar, create the parser, shape and evaluate the parse tree with a Transformer, and optimize using LALR(1) and PyPy.
...
- Parentheses let us group rules together.
- rule\* means optional repetition of any amount. That means, zero or more instances of that rule.
...
You can find the original definitions in common.lark:
https://github.com/lark-parser/lark/blob/master/lark/grammars/common.lark
They don’t strictly adhere to https://json.org/ — but our purpose here is to accept json, not validate it.
...
1. Those little arrows signify aliases. An alias is a name for a specific part of the rule. In this case, we will name the _true/false/null_ matches, and this way we won’t lose the information. We also alias _SIGNED_NUMBER_ to mark it for later processing.
...
3. We turned the _ESCAPED_STRING_ terminal into a rule. This way it will appear in the tree as a branch. This is equivalent to aliasing (like we did for the number), but now _string_ can also be used elsewhere in the grammar (namely, in the _pair_ rule).
...
The first step for optimizing is to have a benchmark. For this benchmark I’m going to take data from http://www.json-generator.com/. I took their default suggestion and changed it to 5000 objects. The result is a 6.6MB sparse JSON file.
...
https://lark-parser.readthedocs.io/examples
