---
title: Using lexer dynamic_complete with Earley and explicit ambiguity
description: Demonstrates how to use lexer='dynamic_complete' and ambiguity='explicit' to parse ambiguous or malformed JSON-like input with underscores instead of quotes, and resolve ambiguities with a custom transformer in Lark.
...
Demonstrates how to use `lexer='dynamic_complete'` and `ambiguity='explicit'` with `parser='earley'`. This example parses JSON-like input where quotes are replaced by underscores: `{_foo_:{}, _bar_: [], _baz_: __}`. Because underscores may legitimately appear inside strings, multiple parses can exist; a custom transformer is used to select a preferred parse.
