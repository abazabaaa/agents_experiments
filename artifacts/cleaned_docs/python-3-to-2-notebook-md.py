...
This standalone script demonstrates how to translate between two trees using Lark's tree templates.
It parses Python 3, translates it to a Python 2 AST, and then outputs the result as Python 2 code.

Requires reconstruct_python.py (from Lark's examples) for generating the final Python 2 code.
Ported from a Jupyter notebook example.
"""

# Imports
# Note: Transformer and v_args are imported for completeness with Lark examples,
# although they are not used directly in this script.
...
def parse_template(s: str):
    """Parse a template string that may include $variables."""
    return parser.parse(s + '\n', start='template_start')


def parse_code(s: str):
    """Parse a full Python file input string (Python 3)."""
    return parser.parse(s + '\n', start='file_input')
...
def translate_py3to2(code: str) -> str:
    """Translate Python 3 source code into Python 2 code using tree templates."""
